<template>
    <div :class="['content', 'order', isMenuExpanded ? 'expanded' : '']">
        ...
        <sales-header :title="$t('Shipments')" active="shipment" :order-id="$route.params.order_id" @onHeaderToggle="toggleHeader($event)"></sales-header>
...
<script>
    ...
    export default {
        ...
        components: { SalesHeader, ShipmentCard },
        ...
        methods: {
            ...
            getShipments (orderId) {
                ...
                this.$http.get('/leagcy-api/shipments', { params: { order_id: orderId, pagination: 0 } })
                    .then(function(response) {
                        EventBus.$emit('hide-ajax-loader');

                        this_this.shipments = response.data.data;
                    })
                    .catch(function (error) {});
            },
